...
module.exports = async (Pendragon, m, commands, chatUpdate) => {
  try {
    ...
    const metadata = m.isGroup ? await Pendragon.groupMetadata(from) : {};
    ...
    const groupAdmin = m.isGroup
      ...
      : [];
    const botNumber = await Pendragon.decodeJid(Pendragon.user.id);
    ...
    global.suppLwa = ""
    global.suppLtel = "https://cutt.ly/PendragonBotSupport";
    ...
    async function doReact(emoji) {
      ...
      await Pendragon.sendMessage(m.from, reactm);
    }
    ...
    //if (body.startsWith(prefix) && !icmd)  return Pendragon.sendMessage(m.from, { text: "Baka no such command" });
    ...
    if (isAntilinkOn && m.isGroup && !isAdmin && !isCreator && isBotAdmin) {
      const linkgce = await Pendragon.groupInviteCode(from);
    ...
    Pendragon.setStatus(upTxt);

    cmd.start(Pendragon, m, {
      name: "Pendragon",
      metadata,
      pushName: pushname,
      participants,
